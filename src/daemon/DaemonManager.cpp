...
bool DaemonManager::start(const QString &flags, NetworkType::Type master, const QString &dataDir, const QString &bootstrapNodeAddress)
{
    ...
    if (master == NetworkType::TESTNET)
        arguments << "--testnet";
    else if (master == NetworkType::STAGENET)
        arguments << "--stagenet";
    ...
    // Start start watcher
    QFuture<bool> future = QtConcurrent::run(this, &DaemonManager::startWatcher, master);
...
bool DaemonManager::stop(NetworkType::Type master)
{
    QString message;
    sendCommand("exit", master, message);
    qDebug() << message;

    // Start stop watcher - Will kill if not shutting down
    QFuture<bool> future = QtConcurrent::run(this, &DaemonManager::stopWatcher, master);
    QFutureWatcher<bool> * watcher = new QFutureWatcher<bool>();
    connect(watcher, &QFutureWatcher<bool>::finished,
            this, [this, watcher]() {
        QFuture<bool> future = watcher->future();
...
bool DaemonManager::startWatcher(NetworkType::Type master) const
{
...
bool DaemonManager::stopWatcher(NetworkType::Type master) const
{
...
bool DaemonManager::running(NetworkType::Type master) const
{ 
    QString status;
    sendCommand("status", master, status);
    qDebug() << status;
...
bool DaemonManager::sendCommand(const QString &cmd, NetworkType::Type master) const
{
    QString message;
    return sendCommand(cmd, master, message);
}

bool DaemonManager::sendCommand(const QString &cmd, NetworkType::Type master, QString &message) const
{
    QProcess p;
